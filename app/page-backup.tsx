...
interface Story {
  ...
  const router = useRouter()tIsMessagingOpen] = useState(false)
  const [currentUser, setCurrentUser] = useState<any>(null)
  const [isLoading, setIsLoading] = useState(true)
      ...
      <header className="glass border-b sticky top-0 z-40">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex gap-4 overflow-x-auto pb-4">
            {/* Add Story */}
            <div className="flex-shrink-0 text-center">
              <div className="w-16 h-16 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 flex items-center justify-center cursor-pointer transition-transform hover:scale-105">
                <Camera className="h-6 w-6 text-white" />
              </div>
              <p className="text-xs mt-1 text-muted-foreground">Your Story</p>
            </div>
